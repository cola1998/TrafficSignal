...
if __name__ == "__main__":
    ...
    for episode in range(Max_episodes):
        ...
        ft_waiting_time,rn = ft_test2()
        ft_waiting_time_list.append(ft_waiting_time)
        ft_remain_car_numbers.append(rn)
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and step < Max_steps:  #
            ...
            waiting_time_l.append(waiting_time_t)
            net_update_times += 1
        ...
        traci.close()
    os.makedirs('policy_save/single_runs/{0}'.format(N))
    writer = SummaryWriter('policy_save/single_runs/{0}/biu_d3qn_{1}'.format(N,N))
    ...
    writer = SummaryWriter('policy_save/single_runs/{0}/loss_biu_d3qn_{1}'.format(N,N))
    for i in range(len(loss_list)):
        writer.add_scalar('loss', loss_list[i], global_step=i)
    writer = SummaryWriter('policy_save/single_runs/{0}/ft_example_{1}'.format(N,N))
    for i in range(len(ft_waiting_time_list)):
        writer.add_scalar('waiting_time', ft_waiting_time_list[i], global_step=i)
        writer.add_scalar('remain_car_number',ft_remain_car_numbers[i],global_step=i)

    writer = SummaryWriter('policy_save/single_runs/{0}/detail_biu_d3qn_{1}'.format(N,N))
    for i in range(len(waiting_time_l)):
        writer.add_scalar('detail_waiting_time', waiting_time_l[i], global_step=i)
        writer.add_scalar('detail_reward', reward_l[i], global_step=i)
    writer = SummaryWriter('policy_save/single_runs/{0}/ft_example_{1}'.format(N,N))
    for i in range(len(ft_waiting_time_list)):
        writer.add_scalar('waiting_time', ft_waiting_time_list[i], global_step=i)
        writer.add_scalar('remain_car_number', ft_remain_car_numbers[i], global_step=i)
